...
#' database. Alternatively the package `RPostgres` (`connect_db()`) or
#' `RPostgreSQL` (`connect_db2()`) can be used.
#' 
...
#' @export connect_db2
#' 
connect_db2 <- function(dbname="", host="localhost", port="5432", user="",
		password="", ...) {
	# Top level
	tt <- tktoplevel()
	tkwm.title(tt, "Connect Database")
